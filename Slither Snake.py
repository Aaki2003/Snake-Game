...
FPS = 5    #the screen gets refreshed 5 times per second
blockSize = 20
noPixel = 0
...
def gameLoop():
    ...
    while not gameExit:
        while gameOver==True: #Loop for multiplle lives/chances to play the game
            ...
            for event in pygame.event.get():
                ...
                if event.type==pygame.KEYDOWN:
                    if event.key ==pygame.K_q:  #q-quit , c-continue
                        gameExit = True
                        gameOver = False
                    if event.key == pygame.K_c:
                        gameLoop()               # game starts again
...
# logic 2 checks for the boundary
...
# logic 4 drawing the snake
        snake(blockSize, snakelist)
        pygame.display.update()

#logic 5 checking where the apple is
